...
orderRouter.put(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (order) {
      ...
      try {
        const reply_to = order.email;
        const subject = `New order ${order._id}`;
        const sent_from = process.env.EMAIL_USER;
        const sent_to = `${order.user.name} <${order.user.email}>`;
        const message = payOrderEmailTemplate(order);
        await sendMail(subject, message, sent_to, sent_from, reply_to);
      } catch (err) {
        console.log("err from outlook", err.message);
      }
